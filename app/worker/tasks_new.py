...
from app.services.media_orchestrator_new import MediaOrchestrator
...
def update_unpublished_status():
    ...
    if unpublished_items:
        ...
        if any(orchestrator.get_all_rem_quota(db).values()):
            sources = db.query(Source.id).filter(
                Source.is_active == True
            ).all()
            for source in sources:
                scan_source(source, db)
...
def download_media(self, media_id: int):
    ...
    try:
        ...
        db.rollback()
        orchestrator.advance_item(db, item, "fail")
