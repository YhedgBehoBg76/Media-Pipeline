...
class MediaOrchestrator:
    NEXT_TRIGGER = {
        MediaStatus.PENDING.value : "start_download",
        MediaStatus.DOWNLOADED.value : "start_segment",
        MediaStatus.SOURCE.value: "",
        MediaStatus.SEGMENTED.value: "start_processing",
        MediaStatus.PROCESSED.value: "start_upload",
    }
    ...
    def next_state(self, db_session, media_id):
        ...
        next_trigger = self.NEXT_TRIGGER.get(item.status.value)
        if not next_trigger:
    ...
    def advance_item(self, db, media: MediaItem, trigger_name: str = None):
        sm = MediaItemSM.MediaStateMachine(model=media, state_attr="status")

        if not trigger_name:
            trigger_name = self.NEXT_TRIGGER.get(sm.current_state.value)

        if not trigger_name:
            logger.warning(f"Cannot find next trigger for MediaItem, id={media.id} with state: '{sm.current_state.value}'")
    ...
    def _start_segment(self, media_id, db_session):
        ...
        segmenter = self.platforms_config.get("segmenter")
        segmenter_params = self.segmenter_config.get(segmenter)
        constraints = self._resolve_constraints(platforms)

        segmenter_params = segmenter_params.update(self.segmenter_params)
        segmenter_params["output_path"] = segmenter_params["output_path"].format(media_id=media_id)

        segmenter_params = segmenter_params.update(constraints)

        self.advance_item(db_session, item)
        self.segment_task.delay(media_id, segmenter, segmenter_params)

    def _start_processing(self, media_id, db_session):
        item = db_session.query(MediaItem).filter(
            MediaItem.id == media_id
        ).with_for_update().first()

        input_path = item.local_path
        output_path = f"{self.TASKS_PATH}/{media_id}/segments/processed_{media_id}_segment_{int(datetime.now(timezone.utc).timestamp())}.mp4"

        if not os.path.isfile(input_path):
            self.advance_item(db_session, item, "fail")
    ...
    def _start_upload(self, media_id, db_session):
        ...
        self.advance_item(db_session, item)
        self.upload_task.delay(local_path, params)
